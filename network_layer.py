...
class IPSocket():
    ...
    def build_ip_packet(self, data):
        ...
        total_len = length_of_ip_header + 20 # length of IP header + length of TCP header (20)
        ID = self.ID
        flags = [0, 0, 0] # the 3 flags for the ip header
        frag_offset = 0
        TTL = 64 # some number
        ...
        header_checksum = calculate_checksum(
            struct.pack(
                ...
                ID, (flags[0] << 7) + (flags[1] << 6) + (flags[2] << 5) + frag_offset,
                TTL, protocol, 0,
    ...
    def receive(self):
        ...
        while received == None:
            ...
            if self.source_ip_address == parsed_ip_header["dest_ip_address"] and self.dest_ip_address == ip_header["source_ip_address"] and calculate_checksum(ip_header) == 0:
